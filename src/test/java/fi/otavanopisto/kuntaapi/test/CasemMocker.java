...
public class CasemMocker extends AbstractMocker {
  ...
  public CasemMocker mockSubnodes(Integer rootNode, Integer...subNodes) {
    ...
    mockGetString(String.format("%s/%s", URL_PREFIX, path), "application/json", readCasemFile(file));
    
    return this;
  }
  
  public CasemMocker mockContentList() {
    mockGetString(String.format("%s/Contents()", URL_PREFIX), "application/json", readCasemFile("contents.json"));
    mockGetString(String.format("%s/Contents", URL_PREFIX), "application/json", readCasemFile("contents.json"));
    return this;
  }
  ...
  public CasemMocker mockContent(Integer... contentIds) {
    for (Integer contentId : contentIds) {
      mockGetString(String.format("%s/Contents(%d)", URL_PREFIX, contentId), "application/json", readCasemFile(String.format("contents/%d.json", contentId)));
      mockGetString(String.format("%s/Contents(%d)/ExtendedProperties", URL_PREFIX, contentId), "application/json", readCasemFile(String.format("contents/%d_ext.json", contentId)));
    }
