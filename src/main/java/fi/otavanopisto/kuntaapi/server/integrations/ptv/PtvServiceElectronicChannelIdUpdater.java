...
public class PtvServiceElectronicChannelIdUpdater extends EntityUpdater {
  ...
  public void timeout() {
    if (systemSettingController.isNotTestingOrTestRunning()) {
      ...
      } else if (serviceElectronicChannelsTaskQueue.isEmpty()) {
...
//    }
//    
//    ApiResponse<List<ElectronicServiceChannel>> response = ptvApi.getServicesApi().listlistServiceElectronicChannels(ptvServiceId.getId(), null, null);
//    if (response.isOk()) {
//      List<ElectronicServiceChannel> electronicChannels = response.getResponse();
//      for (int i = 0; i < electronicChannels.size(); i++) {
//        Long orderIndex = (long) i;
//        
//        ElectronicServiceChannel electronicChannel = electronicChannels.get(i);
